...
#  build_dataset(filename, sequence_length, alphabet_size, max_lines=-1):
#  char_to_one_hot(char, minchar=0, maxchar=128):
#  concat_and_multiply(weights, *args):
#  create_rnn_params(input_size, state_size, output_size,
#  one_hot_to_string(one_hot_matrix):
#  rnn_log_likelihood(params, inputs, targets):
#  rnn_predict(params, inputs):
#  sigmoid(x):
#  string_to_one_hot(string, minchar=0, maxchar=128):
#  strings_to_one_hot(lines, minchar=0, maxchar=128):
#  strings_to_one_hot_(lines, minchar=0, maxchar=128):
...
def string_to_one_hot(string, minchar=0, maxchar=128):
    ascii = np.array([(ord(c)-minchar) for c in string]).T
    return np.array(ascii[:,None] == np.arange(maxchar-minchar)[None, :], dtype=int)

def strings_to_one_hot(lines, minchar=0, maxchar=128):
    """ lines: list of strings (must all have same length) """
    line_length = len(lines[0])
    num_lines = len(lines)
    hot_length = maxchar - minchar
    seqs = np.zeros((line_length, num_lines, hot_length), dtype=int)
    for ix, line in enumerate(lines):
        padded_line = (line + " " * line_length)[:line_length]
        seqs[:, ix, :] = string_to_one_hot(padded_line, minchar, maxchar)[:,:hot_length]
    return seqs

#    alternative implementation, for sake of better understanding the assignment "seqs[:, ix, :] = string_to_one_hot( ... )" 
def strings_to_one_hot_(lines, minchar=0, maxchar=128):
    """ lines: list of strings (must all have same length) """
    line_length = len(lines[0])
    num_lines = len(lines)
    hot_length = maxchar - minchar
    w = np.array(list(map(lambda x: [z for z in x], lines))).T.flatten()
    seqs = np.array(list(map(lambda x: [char_to_one_hot(z,minchar,maxchar) for z in x], w))).reshape(line_length, num_lines, hot_length)
    return seqs
